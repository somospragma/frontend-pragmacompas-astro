...
describe("MentorshipForm", () => {
  ...
  describe("Data Loading", () => {
    ...
    it("should handle partial data loading failures gracefully", async () => {
      (getSkills as Mock).mockRejectedValue(new Error("Skills error"));
      (getChapters as Mock).mockResolvedValue(mockChapters);
  ...
  describe("Form Submission", () => {
    ...
    it("should redirect to history page on successful submission", async () => {
      const user = userEvent.setup();
      const mockHref = "/history";
      vi.spyOn(ROUTE_PATHS.HISTORY, "getHref").mockReturnValue(mockHref);
  ...
  describe("Loading States", () => {
    it("should disable submit button while submitting", async () => {
      ...
      (createTutoringRequest as Mock).mockImplementation(mockDelay);
      const user = userEvent.setup();
      render(<MentorshipForm />);
  ...
  describe("Accessibility", () => {
    ...
    it("should have sr-only text for submitting state", async () => {
      const mockDelay = () => new Promise((resolve) => setTimeout(resolve, 100));
      (createTutoringRequest as Mock).mockImplementation(mockDelay);
      const user = userEvent.setup();
      render(<MentorshipForm />);
    ...
    it("should have error state with proper ARIA live region", async () => {
      (getSkills as Mock).mockRejectedValue(new Error("Network error"));
      (getChapters as Mock).mockRejectedValue(new Error("Network error"));
  ...
  describe("User Interactions", () => {
    it("should update textarea value on user input", async () => {
      ...
      const textarea = screen.getByLabelText(/notas/i) as HTMLTextAreaElement;
      await user.type(textarea, "Test input");

      expect(textarea.value).toBe("Test input");
    });

    it("should reload page when clicking reload button on error", async () => {
      (getSkills as Mock).mockRejectedValue(new Error("Network error"));
      (getChapters as Mock).mockRejectedValue(new Error("Network error"));

      const reloadMock = vi.fn();
      (globalThis as { location: { reload: () => void } }).location.reload = reloadMock;
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle empty chapters array", async () => {
      (getChapters as Mock).mockResolvedValue([]);
    ...
    it("should clear validation errors when correcting fields", async () => {
      const user = userEvent.setup();
      render(<MentorshipForm />);
